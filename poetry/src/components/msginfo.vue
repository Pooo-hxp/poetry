  ...
  <div>
    <ol class="container msg-list">
      <li class v-for="(item,index) in list" :key="index">
        <div class="container hxp-msg-list">
          ...
          <p></p>
          <p class="col-md-4" style="height: 4rem; overflow: auto;">{{item.Msg}}</p>
          <p class="text-right col-md-4"></p>
          <div class="text-right hxp-msg-list-icon">
            <a href @click.prevent="like(item.MsgID,item.Like,index)">
              <img src="../assets/images/icon/赞(前).png" />
              （{{item.Like}}）
            </a>
            <!-- 
            ...
            <a href @click.prevent="sick(item.MsgID,item.Sick,index)">
              <img src="../assets/images/icon/踩(后).png" />
              （{{item.Sick}}）
            </a>
          </div>
        </div>
    ...
    <!-- 利用positon:absolute/z-index:-1使其固定于最底层 -->
    <div class="maskingcolor"></div>
    <!-- 利用positon:absolute/z-index:-2使其固定于最底层 -->
  </div>
</template>
    <script>
export default {
  ...
  methods: {
    like: function(id, like, index) {
      $("this>img").attr("src", "assets/images/icon/踩(后).png'"); 
      console.log(`点击的当前评论的ID为${id}`);
      like < 99 ? (like = like * 1 + 1) /* string->number */ : (like = "99+");
  ...
  components: {},
  mounted() {
    this.$axios.get("https://www.xipengheng.cn/AAA/selectMsg.php").then(res => {
